...
/*
            ...
            Pack[] packs = new Pack[game.getPlayersCount()];
            for (int i = 0; i < game.getPlayersCount(); i++) {
                packs[i] = new Pack();
                for (TrickListIterator rli = game.getTrickList().iterator(); rli.hasNext();) {
                    Trick trick = rli.next();
                    packs[i].add(trick.getPlayerCard(game.getPlayer(i)));
                }
                ...
                if (announce.isColorAnnounce()) {
                    packs[i].arrangeCL(AnnounceUnit.transformFromAnnounceSuitToSuit(announce.getAnnounceSuit()));
                }
            }

            String count = "" + gameCount;
            int n = 7 - count.length();
            for (int i = 0; i < n; i++) {
                count = "0" + count;
            }
...
//            String fileName = "LOG/game_" + count + "_" + announce.toString() + ".htm";
            ...
            sb.append("<tr><td>");
            //sb.append("<b>" + game.getAnnounceList().get .getAnnounceText() + "</b>");
            sb.append("</td></tr>");
            sb.append("</table>");

            sb.append("<br>");
            sb.append("<table width = '100%' border='1' bgcolor = '#FFE6CC'>");
            sb.append("<tr>");
            for (int i = 0; i < game.getPlayersCount(); i++) {
                sb.append("<td width='25%'>");
                sb.append("<b>" + game.getPlayer(i).toString() + "</b> ");
                sb.append("</td>");
            }
            sb.append("</tr>");
            sb.append("<tr>");
            for (int i = 0; i < game.getPlayersCount(); i++) {
                sb.append("<td width='25%'>");
                sb.append("<b>P: " + game.getPlayer(i).getPreferredSuits().toHTMLString() + "</b> ");
                sb.append("</td>");
            }
            sb.append("</tr>");
            sb.append("<tr>");
            for (int i = 0; i < game.getPlayersCount(); i++) {
                sb.append("<td width='25%'>");
                sb.append("<b>U: " + game.getPlayer(i).getUnwantedSuits().toHTMLString() + "</b> ");
                sb.append("</td>");
            }
            sb.append("</tr>");
            sb.append("<tr>");
            for (int i = 0; i < game.getPlayersCount(); i++) {
                sb.append("<td width='25%'>");
                sb.append("<b>M: " + game.getPlayer(i).getMissedSuits().toHTMLString() + "</b> ");
                sb.append("</td>");
            }
            sb.append("</tr>");
            sb.append("</table>");
            ...
            for (TrickListIterator rli = game.getTrickList().iterator(); rli.hasNext();) {
                Trick round = rli.next();
                Player player = round.getAttackPlayer();
                ...
                for (int i = 0; i < game.getPlayersCount(); i++) {
                    sb.append("<td width='25%'>");
                    sb.append("<b>" + player.toString() + "</b> ");
                  //  sb.append(packs[player.getID()].toHTMLString());
                    sb.append("</td>");
                ...
                sb.append("<tr>");
                for (PackIterator pi = round.iterator(); pi.hasNext();) {
                    Card card = pi.next();
                    sb.append("<td width='25%'>");
                    sb.append("<b>" + card.getCardAcquireMethod() + "</b>");
                    sb.append("</td>");
                }
                sb.append("</tr>");
                ...
                for (PackIterator pi = round.iterator(); pi.hasNext();) {
                    Card card = pi.next();
                    sb.append("<td width='25%'>");
                    sb.append("<b>" + card.toString() + "</b>");
                    sb.append(" - ");
                    sb.append(card.getRank().getRankSign());
                    sb.append(" ");
                    sb.append(card.getSuit().getSuitImageTag());
                    sb.append("</td>");
                }
                sb.append("</tr>");
